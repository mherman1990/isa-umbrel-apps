...
|---|---|---|---|
| **Accounting / financials** | `services/financials.py`, `services/lender_packet.py`, `services/cashflow.py`, `api/v1/financials.py`, `api/v1/leases.py`, `tax_packs/`, `cashflow_packs/`, `frontend/.../money/` | `MoneyTransaction`, `BudgetLine`, `CropYear`, `FieldOperation` (harvest), `OperatingLoan`+`OperatingLoanEvent`, `Lease` | Have (0.2.0 expansion): budget-vs-actual + per-field breakeven with honest "insufficient data"; **Schedule F classification** (versioned tax pack, uncategorized surfaced not guessed); **lender-packet export** (print-to-PDF HTML, discloses the missing balance sheet); **cash-flow projection** (budget × cited timing pack; priced-contract inflow) + **operating-line ledger** (balance derived from draws/paydowns); **enterprise allocation** (`PATCH /transactions/{id}`); **operating-mode scenarios** (own/cash-rent/crop-share — net **and** cash outlay, comparative verdict) with the **`Lease` entity now wired**. Next: QuickBooks adapter (deferred), operating-line interest auto-accrual, per-category cash-flow timing overrides, more tax-year packs, balance-sheet (asset/debt) records to unlock true lender ratios. *Bring: farm accounting / ag lending expertise.* Rule: never invent a number — surface gaps. |
| **GIS / geospatial** | `services/clu_import.py`, `api/v1/fields.py`, `frontend/.../fields/` (`FieldMap.tsx`), PostGIS in `models.py` (`Field.boundary`, `CropYear.boundary`, `CaptureEvent.device_gps`) | `Field`, `Farm`, `CropYear` | Have: farmers.gov CLU shapefile/GeoJSON import (pure-Python pyshp), shapefile export, nearest-field GPS attach (`ST_Distance`), acreage recompute (EPSG:5070); **(0.2.0) Leaflet boundary editor** — draw/edit field outlines (`PUT /fields/{id}/boundary`, `POST /fields`; validated GeoJSON, acres recomputed, `source='manual'`), **opt-in Esri imagery OFF by default** (offline blank canvas otherwise; disclosed in `/system/privacy`). Next: yield/zone layers, planted-vs-FSA acre reconciliation, GART export, **subfield planting boundaries** (`CropYear.boundary` — schema exists, editor could target it next). *Bring: GIS / shapefile / CRS expertise.* Keep pure-Python (no GDAL — arm64); the map lib is frontend npm (leaflet + geoman), no arm64 concern. |
| **Agronomy / conservation** | `services/stacking.py`, `mrv.py`, `program_finder.py`, `compliance.py`, `services/agronomy.py`, `api/v1/agronomy.py`, `region_packs/` | `Practice`, `PracticeEvidence`, `Program`, `EligibilityRule`, `StackingRule`, `EvidenceRequirement` | Have: program finder, stacking/additionality checker (crown jewel), MRV readiness, RUP compliance, Iowa pack; **(0.2.0) decision support**: N-rate/**MRTN** (`/agronomy/n-rate`, vs applied N), **fungicide-ROI** (`/agronomy/fungicide-roi`), **practice economics** (`/agronomy/practice-economics`, reuses the stacking engine). All from **cited pack sections read at compute time** (`mrtn`/`fungicide_roi`/`practice_costs`), flagged `unverified: true` pending human verification (HANDOFF §6). Next: **CI-score estimator (GREET-lite)** — deferred (45Z $ risk, HANDOFF §6); more region packs (needs a `find_programs` region filter first — latent bug); N from fertilize-op product analyses. *Bring: agronomy / conservation-program / NRCS-FSA expertise.* Rule: citations + `verify_by` on everything. |
